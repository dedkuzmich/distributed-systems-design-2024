    ...
    if request.method == "POST":
        ...
        while True:
            for secondary in secondaries:
                if secondary.hostname not in res.keys():  # Skip secondary if already have response from it
                    try:
                        # Send messages list
                        sec_res = requests.post(f"{secondary.url}/process_msg", json = msg_list).json()
